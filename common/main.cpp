...
// Board hand-off hook, called once after the summary line is printed. The
// weak default does nothing; a board file may override it (the T2G CM0+ port
// uses it to release the Cortex-M7 core once the M0+ test run -- and thus its
// exclusive use of the shared UART -- is finished).
extern "C" __attribute__((weak)) void board_all_ops_done(void) {}
